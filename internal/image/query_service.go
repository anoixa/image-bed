...
// ListImages 获取图片列表
func (s *QueryService) ListImages(ctx context.Context, storageType string, identifier string, search string, albumID *uint, isPublic *bool, startTime, endTime int64, sort string, page int, limit int, userID int) (*ListImagesResult, error) {
	if page <= 0 {
		page = 1
	}
	if limit <= 0 {
	...
	list, total, err := s.repo.WithContext(ctx).GetImageList(storageConfigIDs, identifier, search, albumID, isPublic, startTime, endTime, sort, page, limit, userID)
	if err != nil {
